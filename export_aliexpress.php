...
if(isset($_REQUEST['category'])){
		...
		if($_REQUEST['category'] == 'all'){
			$productsQuery = 'SELECT * FROM aliexpress_data ';
			$sql = mysql_query($productsQuery);
		}
		else
		{
			$productsQuery = 'SELECT * FROM aliexpress_data limit '.(($_REQUEST['category']-1)*$paging).', '.$paging;
			$sql = mysql_query($productsQuery);	
		}
		
		
		//echo "<a style='border: 2px solid #000; font-weight: bold; color: #000;' href='export_aliexpress.php'>ВСЕ КАТЕГОРИИ</a> ";
...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
		...
		$objPHPExcel->setActiveSheetIndex(0)
		            ...
		            ->setCellValue('B1', 'street_address')
		            ->setCellValue('C1', 'zip')
		            ->setCellValue('D1', 'country_region')
		            ->setCellValue('E1', 'province_state')
		            ->setCellValue('F1', 'city')
		            ->setCellValue('G1', 'telephone')
		            ->setCellValue('H1', 'mobile phone')
		            ->setCellValue('I1', 'fax')
		            ->setCellValue('J1', 'website')
		            ->setCellValue('K1', 'name')
		            ->setCellValue('L1', 'position');
		$objPHPExcel->setActiveSheetIndex(0)->getStyle("A1:L1")->applyFromArray($title);	
		while($row = mysql_fetch_array($sql)){
			
			//echo "<a href='export_aliexpress.php?category=".$row['category']."'>".$row["title"]."</a> ";				`company_name`,
						
			$objPHPExcel->setActiveSheetIndex(0)
						->setCellValue('A'.$n, $row['company_name'])
			            ->setCellValue('B'.$n, $row['street_address'])
			            ->setCellValue('C'.$n, $row['zip'])
			            ->setCellValue('D'.$n, $row['country_region'])
			            ->setCellValue('E'.$n, $row['province_state'])
			            ->setCellValue('F'.$n, $row['city'])
			            ->setCellValue('G'.$n, $row['telephone'])
			            ->setCellValue('H'.$n, $row['mobile phone'])
			            ->setCellValue('I'.$n, $row['fax'])
			            ->setCellValue('J'.$n, $row['website'])
			            ->setCellValue('K'.$n, $row['name'])
			            ->setCellValue('L'.$n, $row['position']);
...
<style>
	...
	a:hover{
		...
		$categoriesQuery = 'SELECT count(*) FROM aliexpress_data';
		...
		for ($i=0; $i < $pages; $i++) { 
			echo "<a href='export_aliexpress.php?category=".($i+1)."'>"."Выкачать пользователей с ".($i*$paging+1)." по ".(($i+1)*$paging)."</a> ";
		}

			
		
		echo "<a style='border: 2px solid #000; font-weight: bold; color: #000;' href='export_aliexpress.php?category=all'>ВЫКАЧАТЬ ВСЕ</a> ";
